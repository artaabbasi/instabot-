...
def open_comment_section(browser, logger):
    ...
    comment_elem = browser.find_element(
...
def comment_image(browser, username, comments, blacklist, logger, logfolder):
    ...
    try:
        if len(comment_input) > 0:
            ...
            # post comment / <enter>
            (
                browser.find_element(By.XPATH, value='//button[@data-testid="post-comment-input-button"][@type="submit"]').click()
            )
...
def get_comments_on_post(
    ...
    try:
        ...
        if all_comment_like_buttons:
            ...
            open_comment_section(browser, logger)
            time.sleep(2)
            username_xpath = '//h3[@class="_6lAjh "]/div/a'
            usernames = browser.find_elements(
                By.XPATH, username_xpath
            )
            comment_xpath = '//div[@class="MOdxS "]/span'
            texts = browser.find_elements(
                By.XPATH, comment_xpath
            )
            for value in usernames:
                commenter = value.text
...
def is_commenting_enabled(browser, logger):
    ...
    comments_disabled = getMediaData("comment_threading_enabled", browser)
